      ...
      Object.values(balances).forEach((balance) => {
        if (balance.token) {
          if (filterSymbolAddress.includes(balance.token.l2Address)) {
            stats[wallet].balances[balance.token.symbol] = getBalance(
              balance.balance,
...
async function getTxs(wallet) {
  ...
  while (!isAllTransfersCollected) {
    await axios
      .get(apiUrl + "/address/" + wallet + "/transfers", {
        params: {
          limit: 100,
          page: pageTransfers,
        },
          ...
          if (
            ...
            if (stableSymbol.includes(transfer.token.symbol)) {
              let amount =
                ...
                Math.pow(10, transfer.token.decimals);
...
async function lite(wallet) {
  await axios
    .post("https://api.zksync.io/jsrpc", {
      id: 1,
      jsonrpc: "2.0",
      method: "account_info",
      params: [wallet],
    ...
    .then((response) => {
      stats[wallet].lite_eth = getBalance(
        parseFloat(response.data.result?.committed?.balances?.ETH || 0),
        18
      );
      stats[wallet].lite_tx = parseInt(
        response.data.result?.committed?.nonce || 0
      );
    });

  await axios
    .get(`https://api.zksync.io/api/v0.2/accounts/${wallet}/transactions`, {
      params: {
        from: "latest",
        limit: "1",
        direction: "older",
    ...
    .then((response) => {
      if (response.data.result.list.length) {
        stats[wallet].lite_last_tx = new Date(
          response.data.result.list[0].createdAt
        );
      }
    });
}
...
async function fetchWallet(wallet, index) {
  ...
  row = {
    ...
    Volume: stats[wallet].volume
      ? "$" + stats[wallet].volume?.toFixed(2)
      : "$" + 0,
  ...
  if (!args.includes("no-lite")) {
    row["Lite ETH"] = stats[wallet].lite_eth
      ? stats[wallet].lite_eth.toFixed(4) + ` ($${usdLiteEthValue})`
      : 0;
    row["Lite TX"] = stats[wallet].lite_tx ? stats[wallet].lite_tx : 0;
    row["Lite last TX"] = stats[wallet].lite_last_tx
      ? moment(stats[wallet].lite_last_tx).format("DD.MM.YY")
      : "";
  }
...
async function addTotalRow() {
  ...
  if (!args.includes("no-lite")) {
    row["Lite ETH"] =
      total.lite_eth.toFixed(4) +
      ` ($${(total.lite_eth * ethPrice).toFixed(2)})`;
  }
